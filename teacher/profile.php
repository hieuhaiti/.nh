...
<body>
    ...
    <div class="form-wrapper">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            <div class="mb-3 row">
                ...
                <div class="col-sm-10">
                    <input type="full_name" class="form-control" name="full_name" id="full_name" value="<?= $CURRENT_USER_INFOR['real_name'] ?>" readonly>
                </div>
                <br><br>
                <!-- email -->
    ...
    if (isset($_POST['update_profile'])) {
        ...
        if ($_FILES['avatar']['name'] != "") {
            ...
            $sql = "UPDATE `user` SET `email`='$email',`password`='$password',`phone_number`='$phone_number',`avatar`='$new_file_name'
                            WHERE user_id=$CURRENT_USER_INFOR[user_id]";
            excute($sql);
            unset($_SESSION['email']);
        } else {
            $sql = "UPDATE `user` SET `email`='$email',`password`='$password',`phone_number`='$phone_number'
                WHERE user_id=$CURRENT_USER_INFOR[user_id]";
            excute($sql);
            unset($_SESSION['email']);
        }
